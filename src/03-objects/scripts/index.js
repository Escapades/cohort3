...
idGridContainer.addEventListener('click', (event) => {
    ...
    if (event.target.id === "idAddAccount") {
        if (idNewAccountName.value === "") {
            idAccountMessage.textContent = "Enter an account name.";
        } else {
            ...
            if (accountController.getAccount(accountName) !== undefined) {
                idAccountMessage.textContent = `${accountName} selected earlier. Choose another name.`;
            }
            else {
    ...
    if (event.target.id === "idTotalAccounts") {
        idReportMessage.textContent = `Sum Total of All Accounts: $${accountController.totalAccounts()}`;
    }
