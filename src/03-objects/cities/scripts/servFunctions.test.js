...
describe('Class methods update data on server using API CRUD functionality', () => {
    ...
    test('loadData() loads server data into city list', async () => {
        ...
        await servFunctions.postData(url + 'clear');

        await serveFunctions.postData(url + 'add', serverTestData[0]);
        await servFunctions.postData(url + 'add', serverTestData[1]);
        await servFunctions.postData(url + 'add', serverTestData[2]);
        await servFunctions.postData(url + 'add', serverTestData[3]);
        await servFunctions.postData(url + 'add', serverTestData[4]);

        let data = await servFunctions.postData(url + 'all');
        expect(data.status).toEqual(200);
        expect(data.length).toBe(5);
        expect(province.cityList.length).toEqual(0);
        expect(province.getHighestKey()).toEqual(0);

        await servFunctions.loadData(province);
        ...
        // //test error catch
        // expect(await servFunctions.loadData()).toEqual("Error: Failed to load cities from server.");
    });
    ...
    test('addData(city) adds city to server', async () => {
        ...
        await servFunctions.addData(newCity);
        ...
        // //test error catch
        // expect(await servFunctions.addData().toEqual("Error: Server not responding. Failed to add city.");
    });
    ...
    test('updateData(city) updates city pop in server', async () => {
        ...
        servFunctions.updateData(province.getCity(7));

        data = await servFunctions.postData(url + 'all');
        expect(data.status).toEqual(200);
    ...
    test('deleteData(key) deletes city in server by key', async () => {
        await servFunctions.postData(url + 'clear');
        await servFunctions.postData(url + 'add', serverTestData[0]);
        await servFunctions.postData(url + 'add', serverTestData[1]);
        await servFunctions.postData(url + 'add', serverTestData[2]);
        await servFunctions.postData(url + 'add', serverTestData[3]);
        await servFunctions.postData(url + 'add', serverTestData[4]);
        ...
        await servFunctions.deleteData(2);
