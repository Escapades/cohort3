...
import functionsView from './functionsView.js'
...
document.addEventListener('DOMContentLoaded', async () => {
    const errorMessage = await servFunctions.loadData(province);
    if (errorMessage) {
        idFetchError.textContent = errorMessage;
    } else {
        functionsView.refreshMap(province.cityList);
        if (province.cityList.length > 0) {
            idReports.classList.remove("hidden");
        } else {
            idFetchError.textContent = "Warning:  is empty."
        }
    }
});
...
idSummaryPanel.addEventListener('click', async (event) => {
    ...
    if (event.target.id === "idAddCity") {
        if (idCityName.value === "") {
            ...
            const errorMessage = await servFunctions.addData(newCity);
            if (errorMessage) {
                idFetchError.textContent = errorMessage;
...
idMapPanel.addEventListener('click', async (event) => {
    ...
    if (event.target.nodeName === "circle") {
        functionsView.selectPoint(event.target);
    ...
    if (event.target.id === "idDelete") {
        const errorMessage = await servFunctions.deleteData(selectedCityKey);
        if (errorMessage) {
            idFetchError.textContent = errorMessage;
        } else {
    ...
    if (event.target.id === "idMovedIn") {
        if (idPopChange.value !== "") {
            ...
            const errorMessage = await servFunctions.updateData(selectedCity);
            if (errorMessage) {
                idFetchError.textContent = errorMessage;
